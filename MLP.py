...
class NeuralNetMLP(object):
    ...
    def __init__(self, n_output, n_features, n_hidden=30, l1=0.0, l2=0.0, epochs=500, eta=0.001, alpha=0.0,
                 decrease_const=0.0, shuffle=True, minibatches=1, random_state=None, hidden_activation='sigmoid', output_activation='sigmoid'):
    ...
    def _get_gradient(self, a1, a2, a3, z2, y_enc, w1, w2):
        ...
        if self.hidden_activation == 'sigmoid':
            ...
            sigma2 = w2.T.dot(sigma3) * self._relu_gradient(z2)
...
# --------------------------------------2. Implement Partial_Fit(X, y, epoch) --------------------------------------
    def partial_fit(self, X, y, epochs=1, print_progress=False):
        """
        ...
        y : array, shape = [n_samples]
            Target class labels.
        epochs : int
            Number of additional epochs for training.
        print_progress : bool (default: False)
            Prints progress as the number of epochs to stderr.
        """
        if not hasattr(self, 'w1'):
            self.w1, self.w2 = self._initialize_weights()  # Initialize only if not previously done
        ...
        initial_epoch = len(self.cost_) // len(y)  # Start from the current epoch
        for i in range(initial_epoch, initial_epoch + epochs):
            self.eta /= (1 + self.decrease_const * i)

            if print_progress:
                sys.stderr.write('\rEpoch: %d/%d' % (i + 1, initial_epoch + epochs))
                sys.stderr.flush()
